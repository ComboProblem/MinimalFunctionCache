...
def setup_and_validate_paths():
    paths = {"bkpts_path_base":os.getenv("BKPTS_PATH_BASE"), "rep_elem_path_base": os.getenv("REP_ELEM_PATH_BASE"), "temp":os.getenv("MFC_TEMP")}
    paths["bkpt_path"] = os.path.join(paths["bkpts_path_base"], str(run_params['k']))
    paths["rep_elem_path"] = os.path.join(paths["rep_elem_path"], str(run_params['k']))
    paths["cwd"] = os.getcwd()
    for path in paths:
